...
/*
 * data from http://opendata.epa.gov.tw/Data/Contents/SGWC/
 * http://opendata.epa.gov.tw/ws/Data/SGWC/?format=json
 */
...
$json = json_decode(file_get_contents('http://opendata.epa.gov.tw/ws/Data/SGWC/?format=json'), true);
foreach ($json AS $k => $v) {
    $p = twd97_to_latlng($v['TWD97TM2X'], $v['TWD97TM2Y']);
    $f = new stdClass();
    $f->type = 'Feature';
    $f->properties = $v;
    $f->geometry = new stdClass();
    $f->geometry->type = 'Point';
    $f->geometry->coordinates = array($p['lng'], $p['lat']);
    $points->features[] = $f;
}
file_put_contents(dirname(__DIR__) . '/sgwc/points.json', json_encode($points, JSON_UNESCAPED_UNICODE));

function twd97_to_latlng($x, $y, $base = 121) {
    $a = 6378137.0;
    $b = 6356752.314245;
    $lng0 = $base * M_PI / 180;
    $k0 = 0.9999;
