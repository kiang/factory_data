...
foreach(glob(dirname(__DIR__) . '/factory/T/*.json') AS $jsonFile) {
  $json = json_decode(file_get_contents($jsonFile), true);
  if(!empty($json['egis']['X'])) {
    $p = twd97_to_latlng($json['egis']['X'], $json['egis']['Y']);
    $f = new stdClass();
    $f->type = 'Feature';
    $f->properties = $json['egis'];
...
file_put_contents(dirname(__DIR__) . '/points.json', json_encode($points));
